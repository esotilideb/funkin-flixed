...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		abg = new FlxBackdrop(Paths.image('titlescreen/checkers or something'), #if (flixel < "5.0.0") 1, 1, true, true, #else XY, #end FlxG.random.int(-25, 5), 0);
		#if (flixel >= "5.0.0" )
		abg.scrollFactor.set(0.3, 0.3);
		#end
		abg.screenCenter();
		...
		blackbars = new FlxSprite(0, 0).loadGraphic(Paths.image('titlescreen/lineas'));
		blackbars.scrollFactor.set(0);
		blackbars.setGraphicSize(Std.int(blackbars.width * 0.8));
		blackbars.screenCenter();
		add(blackbars);

		john = new FlxSprite(450, 10);
		john.frames = Paths.getSparrowAtlas('titlescreen/John Intro');
		john.scrollFactor.set(0);
		john.setGraphicSize(Std.int(john.width * 0.7));
		john.antialiasing = false;
		john.animation.addByPrefix('intro', 'John Intro', 10, true);
		john.animation.play('intro');
		add(john);	
