...
class PlayState extends MusicBeatState
{
	...
	public var coolsongName:FlxText;
	var timeTxt:FlxText;
	...
	override public function create()
	{
		...
		if (SONG.stage.toLowerCase() == 'forest')
		...
		coolsongName = new FlxText(-200, timeBarBG.y + 55, FlxG.width, "", 15);
		coolsongName.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		coolsongName.scrollFactor.set();
		coolsongName.visible = false;
		coolsongName.borderSize = 1.25;
		add(coolsongName);

		coolsongName.text = SONG.song;
			...
			if (!paused)
			{
				...
				if(updateTime) {
					...
					if(secondsTotal < 0) secondsTotal = 0;
						...
						if (SONG.song == 'camp-of-duty')
							timeTxt.text = 'Camp Of Duty';

						else if (SONG.song == 'strike counter')
							timeTxt.text = 'Strike Counter';

						else if (SONG.song == 'tank-life')
							timeTxt.text = 'Tank Life';
						//son los legacy, son reales xdxdxd
						if (SONG.song == 'legacy-songs/camp-of-duty'){
							timeTxt.text = 'Camp Of Duty (Legacy)';
						}

						else if (SONG.song == 'legacy-songs/strike counter') {
							timeTxt.text = 'Strike Counter (Legacy)';
						}

						else if (SONG.song == 'legacy-songs/tank-life') {
							timeTxt.text = 'Tank Life (Legacy) | ' + FlxStringUtil.formatTime(secondsTotal, false);
						}

						else if (SONG.song == 'legacy-songs/fortnite')
							timeTxt.text = 'ENCORE SEEEEEEEEEEEEX | ' + FlxStringUtil.formatTime(secondsTotal, false);
						else
		...
		switch(eventName) {
			...
			case 'Duo singing':
				...
				switch(value1.toLowerCase().trim()) {
					case 'on':
						bothAreSinging = true;
						iconP2.changeIcon('duo');
					case 'off':
						bothAreSinging = false;
				}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (Paths.formatToSongPath(SONG.song) == 'tank-life')
					{
			...
			{
				if (Paths.formatToSongPath(SONG.song) == 'camp-of-duty' || Paths.formatToSongPath(SONG.song) == 'legacy-songs/camp-of-duty')
					{
						if (!GameJoltAPI.checkTrophy(194722))
							GameJoltAPI.getTrophy(194722, '#27FF00', 'Camp of duty FC', 'When le ases fc a campo de duti o algo asi nose no soy fan');

							GameJoltAwards.hasCOD = true;
					}

				if (Paths.formatToSongPath(SONG.song) == 'strike-counter' || Paths.formatToSongPath(SONG.song) == 'legacy-songs/strike-counter')
					{
						if (!GameJoltAPI.checkTrophy(194726))
							GameJoltAPI.getTrophy(194726, '#27FF00', 'Strike counter FC', 'No se me ocurre un chiste para esto');

							GameJoltAwards.hasSC = true;
					}

				if (Paths.formatToSongPath(SONG.song) == 'tank-life' || Paths.formatToSongPath(SONG.song) == 'legacy-songs/tank-life')
					{
						if (!GameJoltAPI.checkTrophy(194725)) 
							GameJoltAPI.getTrophy(194725, '#27FF00', 'Tank life FC', 'The final match. ¿¿¡¡IN FULL COMBO!!??');

							GameJoltAwards.hasTL = true;
					}

				if (Paths.formatToSongPath(SONG.song) == 'fortnite')
					{
						if (!GameJoltAPI.checkTrophy(194728))
							GameJoltAPI.getTrophy(194728, '#27FF00', 'Fortnite FC', 'THE FORTNITE BATTLE ROYALE GUY????');

							GameJoltAwards.hasF = true;
					}
        ...
        if (curSong == 'Fortnite')
			{
				if (boingySproingy){
                    if (bouncay){
                  // boyfriend.scale.set(0.9, 1.1);
                    //boyfriend.y = 10 - 5;
                     // FlxTween.tween( boyfriend.scale, {x: 1, y: 1}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});
                     // FlxTween.tween( boyfriend, {y: 450}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});

                //dad.scale.set(1.15, 0.9);
                // dad.y = 172 - 37;
                //   FlxTween.tween( dad.scale, {x: 1, y: 1}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});
                //  FlxTween.tween( dad, {y: 0}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});

					//gf.scale.set(1.45, 1.9);
					// dad.y = 172 - 37;
					//FlxTween.tween( gf.scale, {x: 1.5, y: 1.5}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});
					//FlxTween.tween( gf, {y: gf.y}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});
             ...
             /*         boyfriend.scale.set(1.15, 0.9);
                    ...
                    dad.scale.set(0.9, 1.1);
                	//dad.y = 142 - 37 - 5;
                      FlxTween.tween(dad.scale, {x: 1, y: 1}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});
                      FlxTween.tween(dad, {y: 0}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});

					gf.scale.set(0.9, 1.1);
					//dad.y = 142 - 37 - 5;
					FlxTween.tween(gf.scale, {x: 1, y: 1}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});
					FlxTween.tween(gf, {y: 0}, Conductor.stepCrochet * 0.0019, {ease: FlxEase.quadOut});
                      bouncay = true;*/
                    }
                }
			}    
